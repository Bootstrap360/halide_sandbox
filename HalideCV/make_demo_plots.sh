...
# grab a trace
HL_JIT_TARGET=host-cuda  HL_TRACE=3 HL_TRACE_FILE=$(pwd)/tmp/trace.bin ./build/demo

echo "Generating Videos"

cat tmp/trace.bin | /home/ubuntu/repositories/halide_sandbox/util/build/HalideTraceViz -s 768 1280 -t 5000 -d 10000 -h 4 -f simple 0 255 2 0 1 2 0 0 1 0 0 1 0 0 | avconv -f rawvideo -pix_fmt bgr32 -s 768x1280 -i /dev/stdin -c:v h264 tmp/simple.mp4 
...
# cat tmp/trace.bin | /home/ubuntu/repositories/halide_sandbox/util/build/HalideTraceViz -s 768 1280 -t 5000 -d 10000 -h 4 -f tile_fuse_parallel 0 255 2 0 1 2 0 0 1 0 0 1 0 0 | avconv -f rawvideo -pix_fmt bgr32 -s 768x1280 -i /dev/stdin -c:v h264 tmp/tile_fuse_parallel.mp4
